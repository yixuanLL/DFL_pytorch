    ...
    do
        ...
        do
            ...
            do
                ...
                py_req="python ${cur_path}/main_flamby.py --seed=9 --DRtest=True --grad_perp_norm=${gpn} --local_round=2 --global_round=50 --lr=${l} --batch_size=2 --momentum=${m} --dataset=FLamby --model=mclr ${iid}";
                echo "${py_req}"
                echo "${py_req}">>$logfile
                start_time=$(date +%s)
                output=`${py_req}`;
    ...
    do
        ...
        do
            ...
            do
                ...
                py_req="python ${cur_path}/main_flamby.py --seed=9 --DRtest=True --dp=True --eps=${e} --grad_perp_norm=${gpn} --local_round=2 --global_round=50 --lr=${l} --batch_size=2 --momentum=${m} --eps_2=0.01 --dataset=FLamby --model=mclr ${iid}";
                echo "${py_req}"
                echo "${py_req}">>$logfile
                start_time=$(date +%s)
                output=`${py_req}`;
