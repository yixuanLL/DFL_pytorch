        ...
        do
            ...
            do
                ...
                do
                    ...
                    py_req="python ${cur_path}/main_fed.py --grad_perp_norm=${gp} --eps=${eps[${i}]} --local_round=1 --global_round=500 --lr=${l}  --glr=${glr} --dataset=${dataset} --model=cnn  --sample_ratio=${r} --num_clients=100  --FLalg=FedDPDIFF";
                    echo "${py_req}"
                    echo "${py_req}">>$logfile
                    start_time=$(date +%s)
                    output=`${py_req}`;
