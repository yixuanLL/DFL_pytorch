...
class DrDPOptimizerV5(DPOptimizer):
    ...
    def __init__(self,
        ...
        *,
        ...
        self.log = []
        self.s = 196
        self.last_grad_clean_norm = None
    ...
    def recover_grad(self, g_perp_noisy, alpha_noisy):
        ...
        # if False:
            ...
            norm = torch.stack(last_norm).norm(2)
            # debias
            self.last_grad_norm = last_norm
            last_clean_norm = [torch.mean(g.reshape(len(g), -1).norm(2, dim=-1)) for g in self.grad_samples]
            self.last_grad_clean_norm = last_clean_norm
