...
class DrDPOptimizertest(DPOptimizer):
    ...
    def dr_process(self):
        ...
        g_perp = self.clip_and_sum(gi_perp, self.perp_grad_norm, 'g_perp') 
        ...
        paral_alpha = self.clip_and_sum(paral_alpha, clip_p, 'alpha')
        paral_alpha_clean = copy.deepcopy(paral_alpha)
        self.add_noise_sum(paral_alpha, self.noise_multiplier_2, clip_p) 
    ...
    def decompose_grad(self):
        # if self.last_grad == []:
        if self.proj_base == []:
            return self.grad_samples, [torch.tensor([1., 1.]).to(self.device)]*8
        alpha_i = [torch.sum((p*g).reshape(len(g), -1), dim=1)/torch.norm(g.reshape(len(g),-1), dim=1) for p,g in zip(self.proj_base, self.grad_samples)]
        gi_perp = [g-p*a.reshape([len(p)]+[1]*(len(p.shape)-1)) for p, g, a in zip(self.proj_base, self.grad_samples, alpha_i)]
        if gi_perp == []:
            print(1)
    ...
    def recover_grad(self, g_perp_clean, g_perp, paral_alpha_clean, paral_alpha):
        # if self.last_grad == []:
        if self.proj_base == []:
            g_noisy = g_perp
            self.last_grad = g_perp
        else:
            g_noisy = [gp + 2*a*lg for gp, a, lg in zip(g_perp, paral_alpha, self.last_grad)] #sum of gradients
        for p,gi in zip(self.params, g_noisy):
            if p.summed_grad is not None:
                p.summed_grad += gi
            else:
                p.summed_grad = gi
        self.last_grad = [gi/self.num_sample for gi in g_noisy] # mean of g_boisy
