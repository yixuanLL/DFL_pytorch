...
class DrDPOptimizertest(DPOptimizer):
    ...
    def dr_process(self):
        gi_perp, costheta, per_param_norms, paral_norm = self.decompose_grad()   
        ...
        if self.last_grad != []:
            clip_p = 0.01
            paral_norm = self.clip(paral_norm, clip_p)
            paral_norm = self.add_noise_mean(paral_norm, self.noise_multiplier_2, clip_p) 
        else:
            paral_norm = 0
        g_perp = self.recover_grad(g_perp, paral_norm, costheta) 
    ...
    def decompose_grad(self):
        ...
        paral_norm =  [torch.mean(gn*cos, dim=0) for cos, gn in zip(per_param_norms, costheta)]
        return gi_perp, costheta, per_param_norms, paral_norm 
