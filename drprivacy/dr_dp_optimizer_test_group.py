...
class DrDPOptimizertest(DPOptimizer):
    ...
    def dr_process(self):
        ...
        if self.last_grad != []:
            ...
            partial = int(len(self.grad_samples[0]) * 0.5)
            self.add_noise_mean(paral_alpha, self.noise_multiplier, clip_p, partial) 
        else:
            paral_alpha = 0
            paral_alpha_clean = 0
    ...
    def decompose_grad(self):
        ...
        # per_param_norms = [g[0:partial].reshape(partial, -1).norm(2, dim=-1) for g in self.grad_samples] # norm of per laryer of per sample gradient
        # last_grad_norms = [g.reshape(-1).norm(2, dim=-1) for g in self.last_grad] # norm of per laryer of last gradient
        # costheta = [torch.mean(torch.sum(g[0:partial].reshape(partial, -1)*(lg.reshape(-1)), dim=1)/(g_norm*lg_norm)) for (g, lg, g_norm, lg_norm) in zip(self.grad_samples, self.last_grad, per_param_norms, last_grad_norms)]
        ...
        # gi_perp = [(g[partial:end]-gl) for g, gl in zip(self.grad_samples, gi_paral)] 
        # paral_alpha =  [torch.mean(gn*cos, dim=0) for cos, gn in zip(per_param_norms, costheta)]
        
        # use mean of norm, instead norm of each sample
        param_norms = [torch.mean(g[0:partial].reshape(partial, -1).norm(2, dim=-1)) for g in self.grad_samples]
        last_grad_norms = [g.reshape(-1).norm(2, dim=-1) for g in self.last_grad] # norm of per laryer of last gradient
        costheta = [torch.mean(torch.sum(g[0:partial].reshape(partial, -1)*(lg.reshape(-1)), dim=1)/(g_norm*lg_norm)) for (g, lg, g_norm, lg_norm) in zip(self.grad_samples, self.last_grad, param_norms, last_grad_norms)]
        gi_paral = [gn*cos * torch.tile(lg.unsqueeze(0),[partial]+[1]*len(lg.shape)) for gn, cos, lg in zip(param_norms, costheta, self.last_grad)]
        gi_perp = [(g[partial:end]-gl) for g, gl in zip(self.grad_samples, gi_paral)] 
        paral_alpha =  [gn*cos for cos, gn in zip(param_norms, costheta)]
    ...
    def add_noise_mean(self, cos, noise_multiplier, sensitivity, length): 
        """
        Adds noise to clipped gradients. Stores clipped and noised result in ``p.grad``
        """
...
class DrKFDPOptimizertest(DrDPOptimizertest): # add KF filter
    ...
    def decompose_grad(self):
        ...
        last_grad_norms = [g.reshape(-1).norm(2, dim=-1) for g in self.last_grad_noisy] # norm of per laryer of last gradient
        costheta = [torch.mean(torch.sum(g.reshape(len(g), -1)*(lg.reshape(-1)), dim=1)/(g_norm*lg_norm)) for (g, lg, g_norm, lg_norm) in zip(self.grad_samples, self.last_grad_noisy, per_param_norms, last_grad_norms)]
        gi_paral = [torch.reshape(gn*cos, [len(gn)]+[1]*len(lg.shape)) * torch.tile(lg.unsqueeze(0),[len(gn)]+[1]*len(lg.shape)) for gn, cos, lg in zip(per_param_norms, costheta, self.last_grad_noisy)]
