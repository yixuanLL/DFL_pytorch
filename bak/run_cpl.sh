...
do
    for gpn in ${g_norm[@]}
    do
        py_req="python ${cur_path}/main_lenet5.py --cpl=True --grad_perp_norm=${gpn} --local_round=2 --global_round=200 --lr=${l}";
        echo "${py_req}"
        echo "${py_req}">>$logfile
        start_time=$(date +%s)
        output=`${py_req}`;
...
do
    ...
    do
        for gpn in ${g_norm[@]}
        do
            py_req="python ${cur_path}/main_lenet5.py --cpl=True --dp=True --eps=${e} --grad_perp_norm=${gpn} --local_round=2 --global_round=200 --lr=${l}";
            echo "${py_req}"
            echo "${py_req}">>$logfile
            start_time=$(date +%s)
            output=`${py_req}`;
