#!/bin/bash
#SBATCH --job-name=sgd_fa
#SBATCH --output=out_sgd_fa
#SBATCH --gres=gpu:1
#SBATCH --mem=8GB
cur_path=`pwd`
...
logfile=${cur_path}/logs/log_sgd_fa_$cur_date
    ...
    do
        ...
        do
            ...
            do
                ...
                py_req="python ${cur_path}/main_test.py --seed=${s} --grad_norm=${gn} --local_round=2 --global_round=50 --lr=${l} --batch_size=2 --dataset=FLamby --model=mclr ${k}";                
                echo "${py_req}"
        ...
        do
            ...
            do
                ...
                do
                    ...
                    py_req="python ${cur_path}/main_test.py --seed=${s} --dp=True --eps=${e} --grad_norm=${gn} --local_round=2 --global_round=50 --lr=${l} --batch_size=2  --dataset=FLamby --model=mclr ${k}";
                    echo "${py_req}"
                    echo "${py_req}">>$logfile
                    start_time=$(date +%s)
                    output=`${py_req}`;
